...
### 6. ساخت برنچ جدید
- [ ] **ایجاد برنچ با نام استاندارد**:
  - **⚠️ مهم**: برنچ جدید از روی برنچی که در حال حاضر در آن هستید ساخته می‌شود
  - فرمت: `[type]/[scope]/[short-description]`
  - **type**: feat, fix, refactor, chore, docs
  - **scope**: نام صفحه‌ای که در آن تغییرات انجام می‌شود
  - **short-description**: توضیح کوتاه با کلمات جداشده با خط تیره (-)
  
  مثال‌ها:
  ```bash
  # برنچ جدید از روی برنچ فعلی ساخته می‌شود
  # (اگر در develop هستید، از develop، اگر در main هستید، از main)
  
  # برای افزودن فیچر جدید در صفحه مدیریت کاربران
  git checkout -b feat/user-management/add-role-filter
  
  # برای رفع باگ در صفحه ورود
  git checkout -b fix/login/resolve-token-refresh
  
  # برای بهبود کد در صفحه داشبورد
  git checkout -b refactor/dashboard/improve-chart-performance
  
  # برای تغییرات کلی (بدون صفحه خاص)
