...
export * from "./routes/orders"; // ← Adding
```
...
## 🌟 Real-World Example - Article Endpoint

The **Article (Learning) endpoint** is a complete real-world implementation in our framework package. It demonstrates all best practices and patterns.
...
1. **List with Pagination** - `useAdminLearningListQuery`
2. **Detail View** - `useAdminLearningDetail`
3. **Create/Update** - `useAddAdminLearningList`, `useUpdateAdminLearningList`
4. **File Upload** - `uploadArticleFile`, `uploadArticleFileChunked`
5. **Content Management** - `useSaveArticleContent`
6. **Status Control** - `useAcceptLearningMutation`
7. **Categories & Authors** - `useAdminCategoryListQuery`, `useAuthorsListQuery`

### 📚 Full Documentation

For complete documentation of Article endpoint including:
...
👉 **[Visit Article Endpoint Documentation](./article-endpoint)**

### 🎯 Key Takeaways from Article Endpoint

1. **Organized Structure** - Clear separation of concerns
2. **Full Type Safety** - No `any` types
3. **Generic Hooks** - Reusable patterns
4. **File Upload** - Chunked upload implementation
5. **Cache Management** - Proper query key structure
...
**New endpoint added? Check out [Examples](./examples)! 🚀**
